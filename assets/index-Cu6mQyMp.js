...
@compute @workgroup_size(256) fn computeSomething(
    ...
    var acceleration = vec3<f16>(0.0, 0.0, 0.0);

    for (var i = 0u; i < numBodies; i++) {
        if (i != index) {
            let other : Vertex = objects[i];
            let distance_vec:vec3<f16> = vec3<f16>(other.position - body.position);
            let distance = length(distance_vec);
            let force:f16 = f16(0.067 * body.mass * other.mass) / f16(distance) * f16(distance);
            let direction:vec3<f16> = normalize(distance_vec);
            acceleration += vec3(direction * force);
        }
    }

    body.velocity += vec3f(acceleration);
